...
inputChangeObserver.subscribe((commands) => {
    entities.player.velocity.set(0, commands.has('moveUp') ? -1 : commands.has('moveDown') ? 1 : 0).normalize().scale(playerPaddleInitialSpeed);
});
...
export function launchBall() {
    ball.velocity.set((Math.random() > 0.5 ? 1 : -1) * initialBallSpeed, (Math.random() > 0.5 ? 1 : -1) * 10);
}
...
function moveBallOnAxis(axis, dt) {
...
function boundingRectOverlaps(boundingRectA, boundingRectB) {
